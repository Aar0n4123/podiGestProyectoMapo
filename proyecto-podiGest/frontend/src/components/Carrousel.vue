...
<template>
  <div class="relative w-200 h-120 overflow-hidden rounded-lg shadow-lg">
    ...
    <img :src="images[currentIndex]" alt="Imagen del carrusel"
      class="w-full h-full object-cover transition-opacity duration-500 select-none" draggable="false"
      @dragstart.prevent />
    ...
    <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
      <span v-for="(img, index) in images" :key="index" @click="goTo(index)" class="w-3 h-3 rounded-full cursor-pointer"
        :class="index === currentIndex ? 'bg-white' : 'bg-gray-400'"></span>
    </div>
  </div>
</template>