<template>
  <div class="min-h-screen bg-gray-100 p-8">
    <div class="max-w-4xl mx-auto bg-white shadow-2xl rounded-xl overflow-hidden">
      ...
      <!-- Encabezado -->
      <header class="p-8 text-white text-center bg-gradient-to-r from-teal-500 to-cyan-600">
        <h1 class="text-3xl font-bold tracking-tight">Citas Agendadas</h1>
        <p class="mt-1 text-teal-100">Consulta y gestiona tus citas programadas.</p>
      </header>
      ...
      <div class="p-6">
        ...
        <!-- Estado Vacío -->
        <div v-else-if="citas.length === 0" class="text-center p-12 border border-dashed border-gray-300 rounded-lg bg-gray-50">
          ...
          <router-link to="/" class="mt-6 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-teal-500 hover:bg-teal-600 transition-colors">
            ...
            <transition name="accordion">
              <div v-if="expandedCitaId === cita.id" class="p-5 border-t border-gray-200 bg-gray-50">
                <dl class="space-y-2 text-sm">
                  ...
                  <div
                      v-for="[key, value] in getFilteredDetails(cita)"
                      :key="key"
                  ...
                  <div v-if="Object.keys(getFilteredDetails(cita)).length === 0" class="text-center py-4 text-gray-500">
...
.accordion-leave-active {
  ...
  max-height: 200px; /* Altura máxima razonable para el efecto */
}
